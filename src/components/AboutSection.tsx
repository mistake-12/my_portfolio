...
export default function AboutSection() {
  ...
  // 与 WelcomeSection 在 2.2vh-2.5vh 交叉过渡
  // Welcome: 2.5vh 完全消失
  // About: 2.2vh 开始淡入 → 3.0vh 完全显示
  // 基于典型视口 winH=900: 2.2×900=1980, 3.0×900=2700
  const containerOpacity = useSpring(
    useTransform(scrollY, [1980, 2700], [0, 1]),
    { stiffness: 50, damping: 22, mass: 1 }
  );

  const containerY = useSpring(
    useTransform(scrollY, [1980, 2700], [60, 0]),
    { stiffness: 50, damping: 22, mass: 1 }
  );
  ...
  return (
    <motion.div
      className="absolute inset-0 flex items-center justify-center bg-bg px-4"
      style={{
      ...
      <div className="mx-auto w-full max-w-6xl">
        <div className="grid grid-cols-12 items-center gap-8 md:gap-12">
          ...
          <div className="col-span-12 md:col-span-7">
            <motion.h2
              className="mb-6 font-sans text-4xl font-semibold text-white sm:text-5xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <motion.p
              className="mb-10 max-w-xl font-sans text-lg font-light leading-relaxed text-zinc-400"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
